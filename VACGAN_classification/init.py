...
class Modules():
    def __init__(self, img_size, channels, num_classes, latent_dim, model_name):
        ...
        self.auxC = define_pretrained_model(model_name, num_classes)
    ...
    def init_modules(self, use_cuda, gpu_ids=[]):
        if use_cuda:
            ...
            self.auxC.cuda(gpu_ids[0])
            self.auxC = nn.DataParallel(self.netC, gpu_ids)
...
class Optimizers():
    def __init__(self, module, G_learning_rate, D_learning_rate, C_learning_rate):
        ...
        self.optimizer_auxC = Adam(module.auxC.parameters(), lr=C_learning_rate, betas=(0.5, 0.999))
...
class LrShedulers():
    def __init__(self, optimizer, n_epochs, epoch, decay_epoch):
        ...
        self.lr_scheduler_auxC = LambdaLR(optimizer.optimizer_auxC, lr_lambda=LrLambda(n_epochs, epoch, decay_epoch).step)
