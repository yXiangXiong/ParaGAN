...
def test_gd(module, dataloader, data_c_loader, memory_allocation, opt):
    ...
    for i, batch in enumerate(dataloader.test_loader):
        ...
        z = Variable(torch.cuda.FloatTensor(np.random.normal(0, 1, (opt.batchSize, opt.latent_dim))).cuda(opt.gpu_ids[0]))
        gen_labels = Variable(torch.cuda.LongTensor(np.random.randint(0, opt.num_classes, opt.batchSize)).cuda(opt.gpu_ids[0]))
        ...
        for j in range(opt.batchSize):
            ...
            feature = model_trunc(gen_imgs[j].unsqueeze(0))['semantic_feature'].squeeze().detach().cpu().numpy()
            x.append(gen_labels.data[0].cpu().float().numpy())
            encoding_array.append(feature)

    encoding_array = np.array(encoding_array)
    # encoding_array = encoding_array.reshape(encoding_array.shape[0], encoding_array.shape[1] * encoding_array.shape[2]*encoding_array.shape[3])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', default='covid4', type=str, help='Choose the dataset name for save results')
    parser.add_argument('--generator', type=str, default='checkpoints/covid4/vacgan_convnext_tiny/best_netG.pth', help='generator checkpoint file')
    parser.add_argument('--classifier', type=str, default='checkpoints/covid4/vacgan_convnext_tiny/best_netC.pth', help='classifier checkpoint file')
    parser.add_argument('--project_name', default='vacgan_convnext_tiny', type=str, help='Choose the project name for save results')
    ...
    memory_allocation = MemoryAllocation(opt.cuda, opt.batchSize, opt.channels, opt.size, opt.gpu_ids)
