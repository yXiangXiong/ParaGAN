...
def define_model_trunc(project_name, model):
    model_trunc = None
    if project_name == 'resnet18' or project_name == 'resnet34' or project_name == 'resnet50':
        model = model.module
        model_trunc = create_feature_extractor(model, return_nodes = {'avgpool': 'semantic_feature'})
    if project_name == 'googlenet':
        model = model.module
        model_trunc = create_feature_extractor(model, return_nodes = {'avgpool': 'semantic_feature'})
    if project_name == 'vgg11' or project_name == 'vgg13' or project_name == 'vgg16' or project_name == 'vgg19':
        model = model.module
    ...
    if project_name == 'mobilenet_v2' or project_name == 'mobilenet_v3_small':
        model = model.module
        model_trunc = create_feature_extractor(model, return_nodes = {'classifier.0': 'semantic_feature'})
    if project_name == 'inception_v3':
        model = model.module
        model_trunc = create_feature_extractor(model, return_nodes = {'avgpool': 'semantic_feature'})
    if project_name == 'densenet121':
        model = model.module
        model_trunc = create_feature_extractor(model, return_nodes = {'features.avgpool': 'semantic_feature'})
    if project_name == 'efficientnet_b5' or project_name == 'efficientnet_b4':
        model = model.module
        model_trunc = create_feature_extractor(model, return_nodes = {'avgpool': 'semantic_feature'})
    if project_name == 'convnext_tiny':
        model = model.module
        model_trunc = create_feature_extractor(model, return_nodes = {'avgpool': 'semantic_feature'})
    if project_name == 'mnasnet0_5' or project_name == 'mnasnet1_0':
        model = model.module
        model_trunc = create_feature_extractor(model, return_nodes = {'classifier.0': 'semantic_feature'})
    if project_name == 'wide_resnet50_2':
        model = model.module
        model_trunc = create_feature_extractor(model, return_nodes = {'avgpool': 'semantic_feature'})
    if project_name == 'shufflenet_v2_x1_0':
        model = model.module
        model_trunc = create_feature_extractor(model, return_nodes = {'conv5.1': 'semantic_feature'})
    return model_trunc
