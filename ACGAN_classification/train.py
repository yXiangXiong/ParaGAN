...
def train_validation(module, loss, optimizer, lr_scheduler, trainloader, valloader, memory_allocation, opt):
    ...
    for epoch in range(opt.epoch, opt.n_epochs):  # 0~99
        ...
        for i, batch in enumerate(trainloader.train_loader):
            ...
            validity, pred_label = module.netD(gen_imgs)
            ...
            real_pred, real_aux = module.netD(real_A)
            d_realA_loss = (loss.adversarial_loss(real_pred, memory_allocation.valid) + loss.auxiliary_loss(real_aux, memory_allocation.A_label)) / 2

            real_pred, real_aux = module.netD(real_B)
            d_realB_loss = (loss.adversarial_loss(real_pred, memory_allocation.valid) + loss.auxiliary_loss(real_aux, memory_allocation.B_label)) / 2

            # Loss for fake images
            fake_pred, fake_aux = module.netD(gen_imgs.detach())
            d_fake_loss = (loss.adversarial_loss(fake_pred, memory_allocation.fake) + loss.auxiliary_loss(fake_aux, gen_labels)) / 2
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', default='covid3', type=str, help='Choose the dataset name for save path')
    ...
    parser.add_argument('--project_name', default='acgan_convnext_tiny', type=str, help='Choose the project name for save path')
