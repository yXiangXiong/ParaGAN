...
def define_model_trunc(project_name, model):
    model_trunc = None
    if project_name == 'acgan_resnet18' or project_name == 'acgan_resnet34' or project_name == 'acgan_resnet50' or project_name == 'acgan_resnet101':
    ...
    if project_name == 'acgan_convnext_tiny':
        model = model.module
        model_trunc = create_feature_extractor(model, return_nodes = {'avgpool': 'semantic_feature'})
    if project_name == 'acgan_densenet121':
        model = model.module
        model_trunc = create_feature_extractor(model, return_nodes = {'features.avgpool': 'semantic_feature'})
    if project_name == 'acgan_mnasnet1_0':
        model = model.module
        model_trunc = create_feature_extractor(model, return_nodes = {'classifier.0': 'semantic_feature'})
    if project_name == 'acgan_mobilenet_v2' or project_name == 'acgan_mobilenet_v3_small':
        model = model.module
        model_trunc = create_feature_extractor(model, return_nodes = {'classifier.0': 'semantic_feature'})
    if project_name == 'acgan_inception_v3':
        model = model.module
        model_trunc = create_feature_extractor(model, return_nodes = {'avgpool': 'semantic_feature'})
    if project_name == 'acgan_efficientnet_b5' or project_name == 'acgan_efficientnet_b4':
        model = model.module
        model_trunc = create_feature_extractor(model, return_nodes = {'avgpool': 'semantic_feature'})
    return model_trunc
