...
def c_hinge_loss_visualize(epochs, train_c_mse_loss, result_path):
    plt.style.use("ggplot")
    plt.figure(dpi=200)
    plt.subplot(1, 1, 1)
    plt.title("Epoch C Hinge Loss")
    plt.plot(np.arange(1, epochs+1), train_c_mse_loss, label='train_c_hinge_loss', color='r', linestyle='-', marker='o')
    plt.legend()
    plt.xlabel('Epochs')
    plt.ylabel('C Hinge Loss')
    plt.savefig(result_path + '/c_hinge_loss.png')
