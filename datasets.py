...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        B_path = self.files_B[random.randint(0, len(self.files_B) - 1)] # cyclegan
        item_B = self.transform(Image.open(B_path).convert('RGB'))

        BB_path = self.files_B[index % len(self.files_B)]
        item_BB = self.transform(Image.open(BB_path).convert('RGB'))

        return {'A': item_A, 'B': item_B, 'BB': item_BB, 'A_path': A_path, 'B_path': B_path, 'BB_path': BB_path}
